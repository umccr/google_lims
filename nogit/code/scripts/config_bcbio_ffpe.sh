...
do
  ...
  cp -v /g/data/gx8/projects/std_workflow/merge_ffpe.sh $DATADIR
  cp -v /g/data/gx8/projects/std_workflow/bcbio_system_normalbw.yaml $DATADIR

  sed -i "s|TEMPLATE|$SAMPLES|g" $DATADIR/merge_ffpe.sh

  # bcbio replaces `.` in CSV templates with a `_` when creating directories
  CLEANBATCH=${BATCH//./_}
  CLEAN="$DIRECTORY"data/$CLEANBATCH

  sed -i "s|CLEAN|$CLEAN|g" $DATADIR/merge_ffpe.sh
  sed -i "s|BATCH|$BATCH|g" $DATADIR/merge_ffpe.sh
  sed -i "s|CONFIG|$CLEANBATCH|g" $DATADIR/merge_ffpe.sh
