  ...
  if [ -n "$(ls -A $RUNDIR/final)" ]; then
    ...
    do
      ...
      if [ -n "$(ls -A $RUNDIR/final/$SAMPLE)" ]; then
        ...
        draw_fusions.R --fusions=$FUSION \
            --alignments=$BAM \
            --output=$PDF \
            --annotation=/g/data3/gx8/local/development/bcbio/genomes/Hsapiens/hg38/rnaseq/ref-transcripts.gtf \
            --cytobands=/g/data/gx8/extras/arriba/cytobands_hg38_GRCh38_2018-02-23.tsv \
            --proteinDomains=/g/data/gx8/extras/arriba/protein_domains_hg38_GRCh38_2018-03-06.gff3
        echo "-------- Finished drawing fusions --------"
      else
