...
write_subject <- function(complete, summary) {
  ...
  read1 <- bcbio_subset %>%
    mutate(from = case_when(run <= 80 ~ paste0(fastq, '/', external_sample_id, '_R1_001.fastq.gz'),
                            run > 80 & run <= 93 ~ paste0(fastq, '/', project_name, '/', sample_name, '/', external_sample_id, '_R1_001.fastq.gz'),
  ...
  read2 <- bcbio_subset %>%
    mutate(from = case_when(run <= 80 ~ paste0(fastq, '/', external_sample_id, '_R2_001.fastq.gz'),
                            run > 80 & run <= 93 ~ paste0(fastq, '/', project_name, '/', sample_name, '/', external_sample_id, '_R2_001.fastq.gz'),
