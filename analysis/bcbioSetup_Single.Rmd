...
samples <- samples_gs %>%
  ...
  mutate(subject_id = gsub(' ', '.', external_subject_id)) %>%
  mutate(sample_id = external_sample_id) %>%
...
bcbio <- samples %>%
  ...
  select(illumina_id, fastq, run, project_name, subject_id, sample_id, library_id, sample_name, assay,
         phenotype, row_id) %>%
...
**Note:** This has _not_ been tested for topups where samples information has been captured both in the old _and_ new style (pre/post run 108). 
...
# Create a unique project name
subject_name <- paste0(timestamp, '_', PROJECT, '_', TYPE, '_Distribute')
