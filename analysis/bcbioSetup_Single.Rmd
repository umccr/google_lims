...
samples_gs %>%
  gs_download(ws = 'Sheet1', to = file.path(backup_dir, filename), overwrite = TRUE)
...
write_batch <- function(complete_batch, summary) {
  ...
  write.table(summary,
              file = here::here('output', batch_name, 'data', paste0(batch_name, '.csv')),
              row.names = FALSE,
              col.names = TRUE,
              quote = FALSE,
              sep = ',')
...
### 6. Sync data from Spartan to Raijin
