...
PROJECT <- 'Research-APGI-Garvan-KRAS-wt'
TYPE <- 'WTS'
```
...
read1 <- bcbio %>%
  mutate(from = paste0(fastq, '/', sample_id, '_R1_001.fastq.gz')) %>%
  mutate(to = paste0(targetname, '_R1_001.fastq.gz')) %>%
  select(from, to)

read2 <- bcbio %>%
  mutate(from = paste0(fastq, '/', sample_id, '_R2_001.fastq.gz')) %>%
...
Once the run finishes run the post-processing script. On Raijin, starting from the `work` directory where `run.sh` is placed:

    `cd ..;source /g/data3/gx8/extras/umccrise/load_umccrise.sh;umccrise . -j 16 --cluster-auto`

And on Spartan:

    `cd ..;source /data/cephfs/punim0010/extras/umccrise/load_umccrise.sh;umccrise . -j 16 --cluster-auto`

The post-processing will take abot an hour per sample, mostly due to creating mini-BAM files from the full BAMs.
