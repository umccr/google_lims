...
write_subject <- function(subject, summary) {
  ...
  # Second step, rename
  #
  # Should no longer be needed, now renaming files on the fly
  #
  write.table(paste('mv', 
                    paste(link$runname, link$from_file, sep='/'),
                    link$to,
                    sep = ' '),
              file = here::here('output', subject_name, 'data', paste0(subject_name, '_files.sh')),
              col.names = FALSE,
...
# Group by subject
unique_subjects <- as.vector(unique(bcbio$subject_id))

for (subject in unique_subjects) {
  write_subject(subject, bcbio)
}
```
